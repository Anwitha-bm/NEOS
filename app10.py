...
# --- CONFIGURATION ---
HF_API_TOKEN = "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"  # <-- Replace with your token
TTS_MODEL_ID = "ibm-granite/granite-3.3-2b-base"
PARAPHRASE_MODEL_ID = "tuner007/pegasus_paraphrase"
SUMMARIZE_MODEL_ID = "facebook/bart-large-cnn"   # <-- NEW
...
SUMMARIZE_API_URL = f"https://api-inference.huggingface.co/models/{SUMMARIZE_MODEL_ID}"   # <-- NEW
...
st.write("🎧 Convert text into expressive audiobooks using IBM Granite (Online) or Granite (Offline) with optional paraphrasing or summarization.")
...
if "final_text" not in st.session_state:
    st.session_state.final_text = ""   # <-- NEW: keeps last processed text
        ...
        for i, item in enumerate(reversed(st.session_state.history), 1):
            with st.expander(f"Entry {i} - Tone: {item['tone']} | Engine: {item['engine']}"):
                ...
                if item.get("rewritten_text") and item["rewritten_text"] != item["original_text"]:
                    st.markdown("*Rewritten/Summarized Text:*")
...
def summarize_text(text):
    payload = {"inputs": text, "parameters": {"max_length": 150, "min_length": 40, "do_sample": False}}
    response = requests.post(SUMMARIZE_API_URL, headers=HEADERS, json=payload)
    if response.status_code == 200:
        results = response.json()
        if isinstance(results, list) and "summary_text" in results[0]:
            return results[0]["summary_text"]
        else:
            return text
    else:
        st.error(f"Summarization Error: {response.status_code}")
        st.text(response.text)
        return text
...
if st.button("🔁 Rewrite Text"):
    if not text_data.strip():
        ...
        st.session_state.final_text = rewritten   # <-- NEW
        text_data = rewritten
...
if st.button("📄 Summarize Text"):
    if not text_data.strip():
        ...
        with st.spinner("Summarizing using BART model..."):
            summarized = summarize_text(text_data)
        col1, col2 = st.columns(2)
        with col1:
            st.subheader("Original Text")
            st.write(text_data)
        ...
        st.session_state.history.append({
            ...
            "rewritten_text": summarized,
            "tone": tone,
            "engine": engine_choice
        })
        st.session_state.final_text = summarized   # <-- NEW
        text_data = summarized
    ...
    if not st.session_state.final_text.strip():
        ...
        with st.spinner("Generating audio..."):
            if engine_choice == "IBM Granite (Online)":
                audio_bytes = generate_granite_audio(st.session_state.final_text)   # <-- NEW
                ...
                engine.save_to_file(st.session_state.final_text, temp_audio_path)   # <-- NEW
                engine.runAndWait()
        ...
        if audio_bytes:
            st.session_state.history.append({
                "original_text": text_data,
                "rewritten_text": st.session_state.final_text,
                "tone": tone,
                "engine": engine_choice
            })
            st.subheader("🔊 Audio Output")
            st.audio(audio_bytes, format="audio/wav")
            st.download_button("⬇ Download Audio", data=audio_bytes, file_name="neosonic_output.wav", mime="audio/wav")
        else:
